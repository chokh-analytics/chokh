import { DIMENSIONS, SESSION_DIMENSIONS, type Dimension } from '@chokh/store';
...
import { rawBreakdownPipeline } from './pipelines.js';
...
describe('the adapter never creates an index', () => {
  it('leaves a fresh database with nothing but _id until migrate runs', async () => {
    ...
    for (const name of ['events', 'sites', 'rollups_daily']) {
